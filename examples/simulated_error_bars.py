...
functions = {
    ...
    'p_even_odd': (lambda x: model_to_fit.set_transition_prob(x, probabilities[2]),
                   lambda: (model_to_fit.get_transition_prob()[0], model_to_fit.get_transition_error()[0])),
    'p_odd_even': (lambda x: model_to_fit.set_transition_prob(probabilities[1], x),
                   lambda: (model_to_fit.get_transition_prob()[1], model_to_fit.get_transition_error()[1])),
    'f_even': (lambda x: model_to_fit.set_emission_prob(x, probabilities[3]),
               lambda: (model_to_fit.get_emission_prob()[0], model_to_fit.get_emission_error()[0])),
    'f_odd': (lambda x: model_to_fit.set_emission_prob(probabilities[4], x),
              lambda: (model_to_fit.get_emission_prob()[1], model_to_fit.get_emission_error()[1])),
}
...
for a, name, label in zip(ax, ['start_prob', 'p_even_odd', 'p_odd_even', 'f_even', 'f_odd'],
                          ['$p_{init}$', '$p_{even \\rightarrow odd}$', '$p_{odd \\rightarrow even}$', '$f_{even}$',
                           '$f_{odd}$']):
